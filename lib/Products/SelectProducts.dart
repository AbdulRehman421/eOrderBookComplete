...
class MultiSelectCheckListScreen extends StatefulWidget {
  ...
  const MultiSelectCheckListScreen(
      {Key? key, required this.customer, required this.area, required this.sector})
      : super(key: key);
...
class _MultiSelectCheckListScreenState
  ...
  void _loadMoreItems() async {
    ...
    setState(() {
      controller.text.isNotEmpty ? _searchResult.addAll(newItems) : items
          .addAll(newItems);
  ...
  Future<List<Product>> fetchItems(int offset, int limit) async {
    ...
    int? count = controller.text.isNotEmpty ? Sqflite.firstIntValue(
        await database.rawQuery(
            "SELECT COUNT(*) FROM Product WHERE name1 LIKE  '%${controller
                .text}%'  OR pcode LIKE '%${controller.text}%'")) : Sqflite
        .firstIntValue(await database.rawQuery("SELECT COUNT(*) FROM Product"));

    print("Product count $count");

    List<Map<String, dynamic>> products = controller.text.isNotEmpty
        ? await database.rawQuery(
        "SELECT * FROM Product WHERE name1 LIKE '%${controller
            .text}%' OR pcode LIKE '%${controller
    ...
    for (var product in products) {
      ...
      Product p = Product(
          product['pcode'] ?? "", product['name1'], price, 0, 0, "${product['_id']}",int.parse("${product['balance']}"));
  ...
  onSearchTextChanged(String text) async {
    ...
    for (var userDetail in items) {
      if (userDetail.productName.trim().toLowerCase().contains(
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(appBar: AppBar(elevation: 0,
      ...
      actions: [
        items.isNotEmpty || getSelectedItem().isNotEmpty
          ...
          if (getSelectedItem().isNotEmpty) {
            ...
            child: Visibility(visible: visibility, child: const Text("Save")))
            : const SizedBox()
      ],),
      body: Stack(children: [
        GestureDetector(
          ...
          child: ListView(controller: _scrollController,
            children: [
              Visibility(visible: !visibility,
                child: Card(child: Padding(padding: const EdgeInsets.only(
                    left: 12.0, right: 12, top: 16, bottom: 16),
                  child: Form(key: productsKey,
                      child: Column(mainAxisAlignment: MainAxisAlignment.center,
                        ...
                        children: [
                          ...
                          Visibility(visible: true /*!editing*/,
                            child: Padding(
                              padding: const EdgeInsets.only(bottom: 12),
                              child: TextFormField(controller: skuC,
                                validator: (value) {
                          ...
                          Padding(padding: const EdgeInsets.only(bottom: 12),
                            child: TextFormField(controller: productNameC,
                              validator: (value) {
                                if (value == null || value.isEmpty) {
                              ...
                              decoration: const InputDecoration(
                                  ...
                                  border: OutlineInputBorder()),),),
                          Padding(padding: const EdgeInsets.only(bottom: 12),
                            child: TextFormField(controller: priceC,
                              validator: (value) {
                                if (value == null || value.isEmpty) {
                            ...
                            children: [
                              ...
                              Visibility(visible: false,
                                child: !editing
                                  ...
                                  }, child: const Text("Update")), flex: 10,),),
                            ],)
                        ],)),),),),
              Visibility(visible: visibility,
                child: items.isNotEmpty ? Column(children: [
                  Card(margin: const EdgeInsets.all(12),
                      ...
                      child: Column(crossAxisAlignment: CrossAxisAlignment
                        ...
                        Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                          children: [
                            ...
                            Row(children: [
                              Checkbox(value: showSelected, onChanged: (value) {
                                setState(() {
                                  showSelected = value!;
                                });
                              }),
                  ...
                  Container(margin: const EdgeInsets.all(4),
                    ...
                    child: ListTile(contentPadding: const EdgeInsets.only(
                      ...
                      trailing: SizedBox(width: 80, child: Row(
                        crossAxisAlignment: CrossAxisAlignment.center,
                        mainAxisAlignment: MainAxisAlignment.spaceEvenly,
                        children: [
                          InkWell(child: const Icon(Icons.cancel, size: 28,),
                            onTap: () {
                              controller.text = "";
                              // onSearchTextChanged('');
                              currentPage = 1;
                  ...
                  Visibility(
                    visible: selectedProducts.isNotEmpty && showSelected,
                    child: Column(children: [
                      const Padding(padding: EdgeInsets.all(12.0),
                        child: Text("Selected", style: TextStyle(
                            fontSize: 18, fontWeight: FontWeight.bold),),),
                      ListView.builder(itemBuilder: (context, index) {
                        return ProductItem1(selectedProducts, index);
                      },
                  ...
                  _searchResult.isNotEmpty || controller.text.isNotEmpty
                    ...
                    return InkWell(onTap: () async {
                      ...
                      child: Card(margin: const EdgeInsets.symmetric(
                        ...
                        child: ListTile(// contentPadding: const EdgeInsets.all(12),
                          selected: _searchResult[index].selected,
                          ...
                          title: Row(mainAxisSize: MainAxisSize.max,
                            ...
                            children: [
                                  ...
                                  Visibility(visible: false,
                                    child: Container(
                                      ...
                                      child: Row(mainAxisSize: MainAxisSize.min,
                                        children: [
                                          Text("Qty: ${_searchResult[index]
                                              .quantity}",
                                            style: const TextStyle(
                                                color: Colors.white),),
                                          _searchResult[index].discount != 0
                                              ? const VerticalDivider()
                                              : const SizedBox(),
                                          _searchResult[index].discount != 0
                                              ? Text(
                                            "Dis: ${_searchResult[index]
                                                .discount}",
                                            style: const TextStyle(
                                                color: Colors.white),)
                                              : const SizedBox(),
                              ...
                              Column(children: [
                                const SizedBox(height: 6,),
                                Visibility(visible: false,
                                  child: CircleAvatar(maxRadius: 16,
                                    backgroundColor: Colors.blue,
                                    child: GestureDetector(onTap: () async {
                                      showEditProductDialog(
                                          context, _searchResult[index], index);
                                      ...
                                      child: const Icon(Icons.tune, size: 18,
                                        color: Colors.white,),),),),
                                const SizedBox(height: 6,),
                                !_searchResult[index].selected ? CircleAvatar(
                                  maxRadius: 16,
                                  ...
                                  child: GestureDetector(onTap: () async {
                                    ...
                                    child: const Icon(Icons.add, size: 18,
                                      color: Colors.white,),),) : CircleAvatar(
                                  maxRadius: 16,
                                  backgroundColor: Colors.red,
                                  child: GestureDetector(onTap: () async {
                                    showSearchWarningAlert(context, index, 0);
                                  },
                    ...
                    itemCount: _searchResult.isEmpty ? 0 : _searchResult.length,
                    shrinkWrap: true,
                    physics: const ClampingScrollPhysics(),)
                      : ListView.builder(itemBuilder: (context, index) {
                    return InkWell(onTap: () async {
                      if (!items[index].selected) {
                        showEditProductDialog(context, items[index], index);
                      }
                    },
                      child: Card(margin: const EdgeInsets.symmetric(
                          vertical: 3, horizontal: 6),
                          ...
                          title: Row(mainAxisSize: MainAxisSize.max,
                            ...
                            children: [
                                ...
                                child: Row(children: [
                                  CircleAvatar(maxRadius: 16,
                                    ...
                                    child: GestureDetector(onTap: () async {
                                      ...
                                      priceC.text = "${items[index].price}";
                                      productNameC.text =
                                          items[index].productName;
                                      skuC.text = items[index].sku;
                                    },
                                      child: const Icon(Icons.edit, size: 18,
                                        color: Colors.white,),),),
                                  const SizedBox(width: 10,),
                              ...
                              Column(children: [
                                ...
                                !items
                                    .elementAt(index)
                                    .selected ? CircleAvatar(maxRadius: 16,
                                  backgroundColor: Colors.white,
                                  child: GestureDetector(onTap: () async {
                                    showEditProductDialog(
                                        context, items[index], index);
                                  ...
                                  backgroundColor: Colors.red,
                                  child: GestureDetector(onTap: () async {
                                    showWarningAlert(context, index, 0);
                                  },
                                    child: const Icon(Icons.remove, size: 18,
                                      color: Colors.white,),),),
                                const SizedBox(height: 6),
                  ...
                  if (isLoading)const SizedBox(height: 30,
                      width: 30,
      ...
      floatingActionButton: Visibility(visible: false,
        child: Visibility(visible: visibility,
          child: Container(child: FloatingActionButton.extended(onPressed: () {
            setState(() {
              editing = false;
              if (visibility) {
                visibility = false;
  ...
  Future<void> deleteProduct(int index) async {
    ...
    setState(() {
      items.removeWhere((element) => element.sku == items[index].sku,);
    });
    print(box.values);
  }
  ...
  Future<void> deleteSearchProduct(int index) async {
    ...
    setState(() {
      items.removeWhere((element) => element.id == items[index].id,);
      _searchResult.removeWhere((element) =>
      element.id == _searchResult[index].id,);
    });
    print(box.values);
  }
  ...
  void showWarningAlert(BuildContext context, int index, type) {
    int searchIndex = findProductUsingIndexWhere(
        _searchResult, items[index].id);

    AlertDialog alertDialog = AlertDialog(titlePadding: EdgeInsets.zero,
      title: Container(decoration: const BoxDecoration(color: Colors.blue,
          borderRadius: BorderRadius.only(
              topLeft: Radius.circular(10), topRight: Radius.circular(10))),
        width: double.maxFinite,
        child: const Padding(padding: EdgeInsets.all(8.0),
          child: Center(child: Text(
            "Are you sure?", style: TextStyle(color: Colors.white),)),),),
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.all(Radius.circular(10))),
      content: SingleChildScrollView(child: Container(width: double.maxFinite,
        child: Column(mainAxisSize: MainAxisSize.min,
          children: [
            ConstantWidget.SmallWarningWidget(context),
            Padding(padding: const EdgeInsets.all(8.0),
              child: Text("Do you want to " + (type == 1 ? "add " : "remove ") +
                  "${items[index].productName}?", style: const TextStyle(
                ...
                ElevatedButton(style: ButtonStyle(
                    ...
                    padding: MaterialStateProperty.all(
                        const EdgeInsets.symmetric(
                            vertical: 4, horizontal: 24))), onPressed: () {
                    ...
                    onPressed: () async {
                      setState(() {
                        ...
                        selectedProducts.removeWhere((
                            element) => _searchResult[index].id == element.id);
                      });
                      Navigator.pop(context);
                    },
                    child: const Text("Yes")),
  ...
  void showSelectedWarningAlert(BuildContext context, int index, type) {
    int productIndex = findProductUsingIndexWhere(
        items, selectedProducts[index].id);
    int searchIndex = findProductUsingIndexWhere(
        _searchResult, selectedProducts[index].id);
    AlertDialog alertDialog = AlertDialog(titlePadding: EdgeInsets.zero,
      title: Container(decoration: const BoxDecoration(color: Colors.blue,
          borderRadius: BorderRadius.only(
              topLeft: Radius.circular(10), topRight: Radius.circular(10))),
                    ...
                    onPressed: () async {
                      setState(() {
                        ...
                        if (productIndex != -1) {
                          items[productIndex].selected = false;
                          selectedProducts.removeWhere((
                              element) => items[productIndex].id == element.id);
                        }
                      });
                      Navigator.pop(context);
                    },
  ...
  void showEditProductDialog(BuildContext context, Product product, int index) {
    ...
    AlertDialog alertDialog = AlertDialog(titlePadding: EdgeInsets.zero,
      title: Container(decoration: const BoxDecoration(color: Colors.blue,
        ...
        child: const Padding(padding: EdgeInsets.all(8.0),
          child: Center(child: Text(
            "Add Additional", style: TextStyle(color: Colors.white),)),),),
      shape: const RoundedRectangleBorder(
          borderRadius: BorderRadius.all(Radius.circular(10))),
      content: SingleChildScrollView(child: Container(width: double.maxFinite,
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Expanded(flex: 1,
                child: Padding(padding: EdgeInsets.only(bottom: 12.0, top: 12),
                  child: Text("Quantity: "),),),
              Expanded(flex: 1,
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Expanded(flex: 20,
                      child: TextField(controller: quantityController,
                        keyboardType: TextInputType.number,
                        autofocus: true,
                        maxLines: 1,
          ...
          Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              ...
              Expanded(child: Container(width: double.maxFinite,
                child: TextField(controller: priceC,
                  keyboardType: TextInputType.number,
                  autofocus: true,
                  maxLines: 1,
                  textAlign: TextAlign.center,
                  decoration: const InputDecoration(contentPadding: EdgeInsets
            ...
            children: [
              const Expanded(flex: 1,
                child: Padding(padding: EdgeInsets.only(bottom: 12.0),
                  child: Text("Discount"),),),
              Expanded(child: Container(width: double.maxFinite,
                child: TextField(controller: discountC,
                  keyboardType: TextInputType.number,
                  autofocus: true,
                  maxLines: 1,
                  textAlign: TextAlign.center,
            ...
            children: [
              ...
              Expanded(flex: 1,
                child: Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  children: [
                    Expanded(flex: 20,
                      child: TextField(controller: bonusController,
                        keyboardType: TextInputType.number,
                        autofocus: true,
                        maxLines: 1,
                        textAlign: TextAlign.center,
              ...
              ElevatedButton(style: ButtonStyle(
                  ...
                  onPressed: () async {
                    ...
                    try {
                      ...
                      items[productIndex].discount = int.parse(discountC.text);
                    ...
                    try {
                      if (_searchResult.isNotEmpty) {
                        _searchResult[searchIndex].quantity = int.parse(
                            quantityController.text);
                        _searchResult[searchIndex].discount = int.parse(
                            discountC.text);
                        _searchResult[searchIndex].selected = true;
                        _searchResult[searchIndex].price = double.parse(priceC
                            .text);
                        _searchResult[searchIndex].bonus = int.parse(
                            bonusController.text);
                      }
                    } catch (e) {
                      print(e);
  ...
  Widget ProductItem1(List<Product> selectedProducts, int index) {
    return Card(margin: const EdgeInsets.symmetric(vertical: 3, horizontal: 6),
      child: ListTile(// contentPadding: const EdgeInsets.all(12),
              ...
              child: Row(children: [
                CircleAvatar(maxRadius: 16,
                  ...
                  child: GestureDetector(onTap: () async {
                    ...
                    child: const Icon(
                      Icons.edit, size: 18, color: Colors.white,),),),
                const SizedBox(width: 10,),
                CircleAvatar(maxRadius: 16,
                  backgroundColor: Colors.red,
                  child: GestureDetector(onTap: () async {
                    AlertDialog alert = AlertDialog(title: const Text(
                        'Are you sure?'),
                      content: const Text(
                          'By clicking this button, this product will be deleted'),
                      actions: <Widget>[
                    ...
                    child: const Icon(
                      Icons.delete, size: 18, color: Colors.white,),),),
              ],),),
            const SizedBox(width: 10),
            Expanded(child: Column(
              ...
              children: [
                Visibility(visible: false,
                  child: Container(padding: const EdgeInsets.symmetric(
                    ...
                    child: Row(mainAxisSize: MainAxisSize.min,
                      children: [
                        Text("Qty: ${selectedProducts[index].quantity}",
                          style: const TextStyle(color: Colors.white),),
                        selectedProducts[index].discount != 0
                            ? const VerticalDivider()
                            : const SizedBox(),
                        selectedProducts[index].discount != 0
                            ? Text("Dis: ${selectedProducts[index].discount}",
                          style: const TextStyle(color: Colors.white),)
                            : const SizedBox(),
                      ],),),),
                Row(children: [
                  ...
                  SizedBox(width: 5,),
                  Text("Balance: ${selectedProducts[index].balance}",
                    style: TextStyle(fontSize: 15),),
                ],),
                Row(mainAxisAlignment: MainAxisAlignment.spaceBetween,
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    Row(children: [const Text('Price:', style: TextStyle(
                        fontSize: 13),), Container(child: Padding(
                      padding: const EdgeInsets.symmetric(
                          horizontal: 12.0, vertical: 4),
                      child: Text('${selectedProducts[index].price}',
                        style: TextStyle(fontSize: 13),),),),
                    ],),
                    Row(children: [const Text('Disc:', style: TextStyle(
                        fontSize: 13),), Container(child: Padding(
                      padding: const EdgeInsets.symmetric(
                          horizontal: 12.0, vertical: 4),
                      child: Text('${selectedProducts[index].discount}',
                        style: TextStyle(fontSize: 13),),),),
                    ],),
                    Row(children: [const Text('Bns:', style: TextStyle(
                        fontSize: 13),), Container(child: Padding(
                      padding: const EdgeInsets.symmetric(
                          horizontal: 12.0, vertical: 4),
                      child: Text('${selectedProducts[index].bonus}',
                        style: TextStyle(fontSize: 13),),),),
                    ],),
                    Row(children: [const Text('Qty:', style: TextStyle(
                        fontSize: 13),), Container(child: Padding(
                      padding: const EdgeInsets.symmetric(
                          horizontal: 12.0, vertical: 4),
                      child: Text('${selectedProducts[index].quantity}',
                        style: TextStyle(fontSize: 13),),),),
                    ],),
                  ],),
              ],),),
            Column(children: [
              ...
              Visibility(visible: false,
                child: CircleAvatar(maxRadius: 16,
                  backgroundColor: Colors.blue,
                  child: GestureDetector(onTap: () async {
                    showEditProductDialog(context, _searchResult[index], index);
                  },
                    child: const Icon(
                      Icons.tune, size: 18, color: Colors.white,),),),),
              const SizedBox(height: 6,),
              !selectedProducts[index].selected
                ...
                child: GestureDetector(onTap: () async {
                  ...
                  child: const Icon(
                    Icons.remove, size: 18, color: Colors.white,),),),
              const SizedBox(height: 6,),
            ],)
          ],),),);
  }
